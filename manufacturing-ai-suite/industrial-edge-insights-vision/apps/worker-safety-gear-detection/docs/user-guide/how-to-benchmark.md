...
*   A successful deployment of the Worker Safety Gear Detection application using Docker Compose or Helm, as described in the [Get Started](./get-started.md) guide.
...
[
    {
        "pipeline": "worker_safety_gear_detection_benchmarking",
        "payload": {
            "parameters": {
...
*   `pipeline`: The name of the pipeline to execute (e.g., `worker_safety_gear_detection_benchmarking`).
...
```bash
./benchmark_start.sh -p apps/worker-safety-gear-detection/benchmark_gpu_payload.json -n 4
```

*   `-p apps/worker-safety-gear-detection/benchmark_gpu_payload.json`: Specifies the path to your payload configuration file.
...
```bash
./benchmark_start.sh -p apps/worker-safety-gear-detection/benchmark_gpu_payload.json -t 28.5
```
...
In this guide, you learned how to use the `benchmark_start.sh` script to run performance tests on your Worker Safety Gear Detection application. You can now measure performance for a fixed number of streams or automatically determine the maximum stream density your system can support.
